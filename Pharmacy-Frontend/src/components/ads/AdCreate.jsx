...
export default function AdCreate({ onCreated }) {
  const [form, setForm] = useState({ pharmacyId: '', title: '', description: '', type: 'promo', startDate: '', endDate: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ pharmacyId: '', title: '', description: '', type: 'promo', startDate: '', endDate: '' });
      onCreated?.();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border rounded-lg p-4 space-y-3">
      <h2 className="font-semibold text-gray-700">Add Ad</h2>
      ...
      <input name="title" value={form.title} onChange={handleChange} required placeholder="Title" className="border rounded px-3 py-2 text-sm w-full" />
      <input name="description" value={form.description} onChange={handleChange} placeholder="Description" className="border rounded px-3 py-2 text-sm w-full" />
